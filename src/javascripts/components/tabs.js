...
AppTabs.prototype.activateAndToggle = function (event) {
  ...
  var isTabAlreadyOpen = $currentTarget.getAttribute('aria-expanded') === 'true'
  ...
  if (isTabAlreadyOpen) {
    ...
    nodeListForEach($currentTargetSiblings, function ($tabTrigger) {
      ...
      $tabTrigger.parentNode.classList.remove(tabsItemCurrentClass) // on desktop
      $tabTrigger.parentNode.classList.remove(headingItemCurrentClass) // on mobile
    })
  } else {
    // Reset tabs
    this.resetTabs()
    ...
    nodeListForEach($currentTargetSiblings, function ($tabTrigger) {
      ...
      $tabTrigger.parentNode.classList.add(tabsItemCurrentClass) // on desktop
      $tabTrigger.parentNode.classList.add(headingItemCurrentClass) // on mobile
    })
  }
}
